        ...
        <div class="main-page">
          ...
          <div class="main-page__places" v-if="places" >
            ...
            >
              ...
              <router-link class="m-place__title" :place="place" :to="{ name: 'place', params: { id: place.id }}">{{place.name}}</router-link>
          ...
          if(places){
            places.forEach(function (item) {
              if(item.averageCheck){
                ...
                item.averageCheck_percent = result;
              }

              if(item.review){
                var rating = 0;
                var count = item.review.length;
                item.review.forEach(function (reviews) {
                  rating += reviews.rating;
                });
                var avr = Math.round(rating / count);
                item.averageRating = avr;
              }
            })
          }
          return places;
